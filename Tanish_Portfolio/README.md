...
4. Add `GROQ_API_KEY` and `FRONTEND_ORIGINS` as environment variables
5. A `runtime.txt` pinning `python-3.11.9` is committed in `backend/` —
   ...
   error. If you hit that, also set `PYTHON_VERSION=3.11.9` directly under
   Environment on Render.

**Frontend → Vercel**
1. Import the repo, set the root directory to `frontend`
2. Framework preset: Vite (auto-detected)
3. Add `VITE_API_BASE_URL` — your live Render URL — (and optionally
   `VITE_CALENDLY_URL`) as environment variables

**Wire them together**
Once both are live, set `FRONTEND_ORIGINS` on Render to your real Vercel
URL and `VITE_API_BASE_URL` on Vercel to your real Render URL. Without this
